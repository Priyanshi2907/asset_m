...
def logisticdetails(request):
    if request.method=="GET":
        return render(request,"logistic_details.html")
    
    elif request.method=="POST":
        print("i m here")
        kit_checker=request.POST.get('kit_checker_name') or None
        kit_physical_verifier=request.POST.get('kit_verifier') or None
        logistic_assigner=request.POST.get('logistic_assigner') or None

        logistic_details=LogisticDetails.objects.create(
            kit_checker=kit_checker,
            kit_physical_verifier=kit_physical_verifier,
            logistic_assigner=logistic_assigner
        )
        logistic_details.save()
    ...
    if request.method == 'GET':
        ...
        if request.method == 'POST':
            ...
            for i in asset_inventory_obj:
                if i.asset_transfer == 1 or i.asset_transfer == 2 :
                    return HttpResponse("Please select only approved request...")
                elif i.asset_transfer==4:
                    return HttpResponse("Order is beeen transfered---")
...
def move_to_inventory_email(request):
    if request.method =="GET":
        returndate=request.GET.get('returndate')
        asset_id=request.GET.get('asset_id')
        asset_objs = AssetMaster.objects.filter(id=asset_id) 
        user_id=request.user
        print (returndate)

        subject = 'Asset moved to inventory reminder'
        context = { 
            'returndate': returndate,
            'asset_objs': asset_objs,            
            'user_email_id': user_id,
        }
        html_message = render_to_string('mtv_mail.html', context)
        plain_message = strip_tags(html_message)
    
    
        
        # Create the email message with both HTML and plain text content
        email = EmailMultiAlternatives(subject, plain_message, 'priyanshiprajapati29@gmail.com', ['ghoshshaibal@zoomcom.tv', 'erpadmin@zoomcom.tv','2907priyanshi@gmail.com'])
        #'ghoshshaibal@zoomcom.tv', 'erpadmin@zoomcom.tv'
...
def print_delivery_challan_pdf(request):
    ...
    image = Image(image_path, width=inch*5, height=inch*0.8)
    ...
    paragraph_style = ParagraphStyle(name='Normal', fontSize=15, alignment=1)
    
    paragraph = Paragraph(paragraph_text, paragraph_style)

    # Define the width and height of the box
    width = inch * 10
    height = inch * 5  # Adjust height as needed
...
# Now you can use container_table_data as needed in your document
    ...
    table = Table(table_rows, colWidths=[110,160,280,190,250,200, 150])
...
def delivery_challan_csv(request):
    ...
    img = openpyxl.drawing.image.Image('static/images/zoom_logo.png')

    ws = wb.active
    ws.add_image(img, 'A1')
    print(img)
    # Write the data to the Excel file
    ws.append(queryset_values)
    print("i am here")
    ...
    wb.save('delivery_challan.csv')

    # Create a response object and write the CSV data to it
