...
def remove_asset_from_order(request):
    ...
    asset_to_remove.update(add_to_order = False,confirm_order=False, asset_status="available")
...
def cancel_order(request):
    if request.method == 'POST':
        ...
        for m in all_asset_of_order:
            AssetMaster.objects.filter(pk=m.id).update(
                confirm_order = False,
                add_to_order = False,
