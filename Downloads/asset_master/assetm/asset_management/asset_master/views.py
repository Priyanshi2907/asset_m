...
def parse_date_field(date_str):
    """Parse date string and return a date object or None if invalid."""
    if date_str:
        try:
            return datetime.datetime.strptime(date_str, '%Y-%m-%d').date()
        except ValueError:
            raise ValidationError(f'Invalid date format: {date_str}. It must be in YYYY-MM-DD format.')
    return None
def update_asset(request, id):
    ctx = {}
    asset_obj = AssetMaster.objects.filter(pk=id).first()
    print("asset obj: ",asset_obj.amc_date)
    if not asset_obj:
    ...
    if request.method == 'POST':
        ...
        warranty_period = request.POST.get('warranty_period', None) or None
        ...
        if request.FILES.get('image'):
            ...
            asset_obj.image = image

        asset_obj.model_no = model_no
        print("i m here")
        asset_obj.serial_no = serial_no
        asset_obj.category = category
        asset_obj.sub_category = sub_category
        asset_obj.asset_tag = asset_tag
        asset_obj.brand = brand
        asset_obj.sku = sku
        asset_obj.fc_no = fc_no
        asset_obj.description = description
        asset_obj.status = status
        asset_obj.age = age
        asset_obj.box_number = box_number
        asset_obj.last_service_date = last_service_date
        asset_obj.upcoming_service_date = upcoming_service_date
        asset_obj.length = length
        asset_obj.breadth = breadth
        asset_obj.width = width
        asset_obj.height = height
        asset_obj.warranty_period = warranty_period
        asset_obj.amc_date = amc_date
        asset_obj.storage_warehouse_number = storage_warehouse_number
        asset_obj.outward_date = outward_date
        asset_obj.outward_remarks = outward_remarks
        asset_obj.inward_date = inward_date
        asset_obj.inward_remarks = inward_remarks
        asset_obj.vendor = vendor
        asset_obj.purchased_on = purchased_on
        asset_obj.cost_price = cost_price
        asset_obj.tax_rate = tax_rate
        asset_obj.depricated_value = depricated_value
        asset_obj.created_on = created_on
        asset_obj.rental_pricing = rental_pricing
        asset_obj.rent_collected = rent_collected
        asset_obj.available_for_sale = available_for_sale
        asset_obj.asset_utilization = asset_utilization
        asset_obj.owner = owner

        asset_obj.save()

        if request.FILES.get('image'):
            matching_assets = AssetMaster.objects.filter(
...
def autocomplete_acc_search(request):
    ...
    if selected_value == 'category':
        ...
        return JsonResponse([], safe=False)
#physical verification mail
# def asset_physical_ver_email(request):
   
#     asset_id = request.GET.get('asset_ids')
#     asset_objs = AssetMaster.objects.filter(id__in=asset_ids) 
#     # order_obj = OrderDetail.objects.filter(id=order_id).first() 
#     asset_com_id = request.GET.get('')
#     asset_comment=AssetMasterComment.objects.filter
...
#     plain_message = strip_tags(html_message)
    ...
    email = EmailMultiAlternatives(subject, plain_message, 'priyanshiprajapati29@gmail.com', [order_obj.prepared_by_email, order_obj.approver_email])
...
def add_to_order_in_one_go(request):
    if request.method == 'POST':
        asset_ids = request.POST.getlist('asset_ids[]')
        if asset_ids:
            AssetMaster.objects.filter(pk__in=asset_ids).update( 
                add_to_order=True,
                asset_status="rented_out"
            )
            return JsonResponse({'status': 'success', 'message': 'Assets added to order successfully'})
        else:
            return JsonResponse({'status': 'error', 'message': 'No assets selected'})
    else:
...
def update_comment_again(request):
    ...
    asset_id_obj=AssetMaster.objects.filter(id=id).first()
    print("***",asset_id_obj)
    asset_obj = AssetMasterComment.objects.create(
        # date_time = current_utc_time.astimezone(ist_timezone),
...
class GetGroupWiseAssetAccessories(ListAPIView):
    ...
    def list(self, request, *args, **kwargs):
        ...
        parent_asset = list(AssetMaster.objects.filter(id=asset_id, relation=101).values("id","description"))
        print("***",parent_asset)
        parent_description_list = list(([m['description'] for m in parent_asset]))
        print("----",parent_description_list)
        ...
        #serializer = AssetMasterSerializer(accessories_obj, many=True)
...
def update_comment_again_outsource(request):
    id = request.POST.get('id')
    ctx={}
    asset_id_obj=AssetMaster.objects.filter(id=id).values('id')
    print(asset_id_obj)
    asset_obj = AssetMasterComment.objects.create(
        date_time = datetime.datetime.now(),
        asset_id = asset_id_obj,
